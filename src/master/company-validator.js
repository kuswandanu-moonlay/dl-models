require("should");

module.exports = function (data) {
    data.should.not.equal(null);
    data.should.instanceOf(Object);

    data.should.have.property('code');
    data.code.should.instanceOf(String);

    data.should.have.property('name');
    data.name.should.instanceOf(String);

    data.should.have.property('website');
    data.website.should.instanceOf(String);

    data.should.have.property('industry');
    data.industry.should.instanceOf(String);

    data.should.have.property('phoneNumber');
    data.phoneNumber.should.instanceOf(String);

    data.should.have.property('city');
    data.city.should.instanceOf(String);

    data.should.have.property('information');
    data.information.should.instanceOf(String);
};